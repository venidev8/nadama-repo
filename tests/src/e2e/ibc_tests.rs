...
fn pgf_over_ibc_with_hermes() -> Result<()> {
    ...
    let amount = Amount::native_whole(100).to_string_native();
    let tx_args = [
        "transfer",
        "--source",
        ALBERT,
    ...
    let mut client = run!(test_a, Bin::Client, tx_args, Some(40))?;
...
fn transfer_received_token(
    ...
    let mut client = run!(test_b, Bin::Client, tx_args, Some(40))?;
    client.exp_string("Transaction is valid.")?;
    client.assert_success();
...
fn propose_funding(
    ...
    let pgf_funding = PgfFunding {
        continous: vec![PGFTarget::Ibc(PGFIbcTarget {
